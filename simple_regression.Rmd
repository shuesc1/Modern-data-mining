...
Will a team perform better when they are paid more? Is Billy Beane worth 12 million dollars as predicted?

Read an article: Billion dollar Billy Beane
http://fivethirtyeight.com/features/billion-dollar-billy-beane/   (FiveThirtyEight)

* Data: MLPayData_Total.csv: consists of winning records and the payroll of all 30 ML teams from 1998 to 2014. There are 162 games in each season.
...
1. How does payroll relate to the performance (avgwin)?
...
I put the data in the same folder as the .Rmd file and I use relative path to read data. `as.is=F` tells `R` to read in the numbers as numbers and characters as factors.
...
* Displaying the data: histograms, scatter plots, pairwise scatter plots, etc.
...
Payroll for the first row
```{r}
datapay[1, 1] 
datapay[1, "payroll"] 
```

Get the payroll column two ways:
...
Two ways to do this, base `R` or `dplyr` way, both shown below
```{r eval=T}
...
We want to create a subset of the original dataset without X1998 to X2014 and with the rest of the col names sorted that you may find it useful.
...
data1[order(data1$payroll, decreasing = T), "team"]
```
...
Let the repsonse $y_i$ be the `avgwin` and the explanatory variable $x_i$ be `payroll` ($i = 1, \dots, n=30$).
...
<!-- looking to minimize the residuals -->
<!-- however, this can be swayed by outliers -->
<!-- might be better to consider absolute residuals: L1 loss is more robust, not L2 loss -->

$$\min_{b_0,\,b_1} \sum_{i=1}^{n} (y_i - b_0 - b_1  x_i)^{2}$$

The function `lm()` will be used extensively. This function solves the minimization problem that we defined above. Below we use `avgwin` as the dependent $y$ variable and `payroll` as our $x$.
...
Its also useful to notice the outputs of `myfit0` and that of `summary(myfit0)` are different
```{r, eval=T}
myfit0
```

To summaries the OLS estimate, $\hat{\beta}_0 =$ `r myfit0$coefficients[1]` and $\hat{\beta}_1 =$ `r myfit0$coefficients[2]`.

$$\hat{y}_i = \hat{\beta}_0 + \hat{\beta}_1 x_i = 0.42260 + 0.06137 \cdot x_i$$

To estimate the `avgwin` of Oakland Athletics whose `payroll` is `r datapay$payroll[datapay$team == "Oakland Athletics"]`, 
...
For Oakland Athletics, the real `avgwin` is `r datapay$payroll[datapay$team == "Oakland Athletics"]`. So the residual is

$$\hat{\epsilon}_{\text{Oakland Athletics}} = y_{\text{Oakland Athletics}} - \hat{y}_\text{Oakland Athletics} = 0.5445067 - 0.4742044 = 0.0703023.$$
<!-- is this number too large? need to have sigma in able to know if yes or no -->
...
**HERE is how the article concludes that Beane is worth as much as the GM in Red Sox. By looking at the above plot, Oakland A's win pct is more or less same as that of Red Sox, so based on the LS equation, the team should have paid 2 billion. Do you agree?????**
...
<!-- is this value small enough to say that this is a good line? need a benchmark in able to decide -->

#### Mean Sum of Squares (MSE), Residual Standard Error (RSE)
$\sigma^2$ is estimated by the Mean Sum of Squares (MSE). For simple regression, MSE is defined as:

$$MSE = \frac{RSS}{n-2}.$$

$\sigma$ is estimated by the Residual Standard Error (RSE). For simple regression, RSE is defined as:
...
```{r}
(cor(datapay$avgwin, myfit0$fit))^2 # Square of the cor between response and fitted values
```
...
### Inference for the coefficients: $\hat\beta_1$ and $\hat\beta_0$

Under the model assumptions:

1. $y_i$ ind, normally distributed
2. The mean of $y$ given $x$ is linear
3. The variance of $y$ does not depend on $x$

The LS estimates $\hat\beta_1$ and $\hat\beta_0$ has the following properties:

1. Unbiasedness

$$\textbf{E}(\hat{\beta_1}) = \beta_1$$

2. Normality

$$\hat{\beta_1} \sim \mathcal{N}(\beta_1, \textbf{Var}(\hat{\beta_1}))$$
where 

$$\textbf{Var}(\hat{\beta_1}) = \sigma^{2} [\frac{1}{n}+\frac{\bar{x}^{2}}{\sum_{i=1}^{n} (x_i - \bar{x})^{2}}].$$
...
<!-- 1.96 ~ 2 for 95% confidence interval -->
$$\hat\beta ± 2 \cdot SE(\hat\beta).$$
...
### Confidence and Prediction Intervals for the mean reponse and individual repsonses

#### Confidence Interval 
We use a confidence interval to quantify the uncertainty surrounding the mean of the response, i.e., `avgwin`. For example, for teams like Oakland A's whose `payroll`=.841, a 95% Confidence Interval for the mean of response `avgwin` is
...
We now do 95% & 99% PI for a future $y$ given $x=.841$, a prediction interval
...
From our output above, a 95% prediction interval varies from $.41$ to $.53$ for a team like the Oakland A's. But its `avgwin` is $.54$. So it is somewhat unusual but not that unusual! Also, a 99% prediction interval would contain the true Oakland winning percentage of `r datapay$avgwin[datapay$team == "Oakland Athletics"]`.
...
##### Case II 
A perfect linear model between X and Y but with noise here $y= 2+3x + \epsilon$, $\epsilon$ is iid N(0, var=9). Run this repeatedly
...
Same as that in Case II, but lower error to $\sigma=1$
