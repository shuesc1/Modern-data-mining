...
############ Read: Statistical Sleuth, Chapter 7/8 
...
# datapay <- read.csv("MLPayData_Total.csv", header=T, as.is=T) 
  ...
  # as.is=F: numbers > numbers; characters > factor
...
### Packages (skip this for the moment)  
  ...
  dim(pckgs.all)[1] # about 10,000 packages by now!
...
### Before you do any analysis it is A MUST that we take a look at the data 
# a) Understand the variables: unit, format, unusual values...
# b) Summary statistics: mean, sd, cor…
# c) Displaying the data: hist, scatter plots, pairwise scatter plots…
...
### The data set was manually cleaned already
  sum(is.na(datapay))  # check number of missing values. cautious about this!
...
# Displaying variables
  ...
  datapay$team[which.max(datapay$payroll)] # find the team with the max payroll
  datapay$team[which.min(datapay$payroll)]
...
### Make a scatter plot. 
  ...
  identify(datapay$payroll, datapay$avgwin, labels=datapay$team, cex=0.5, pos=3) 
# label some points
  
  text(datapay$payroll, datapay$avgwin, labels=datapay$team, cex=0.7, pos=1) 
# label all points
...
# THEN we have nice properties about the LS estimates b1, b0.
...
### 95% CI for a future y given x=.841, a prediction interval:
  new <- data.frame(payroll=c(.841))
  CIpred <- predict(myfit0, new, interval="prediction", se.fit=TRUE)
  CIpred    # a 95 prediciton interval varies from .41 to .53 for a team like Oakland A. But
            # its avewin is .54. So it is somewhat unusual but not that unusual!
            # datapay$avgwin[datapay$team == "Oakland Athletics"]
  
  CIpred_99 <- predict(myfit0, new, interval="prediction", se.fit=TRUE, level=.99)
  CIpred_99  # a 99% prediction interval would contain .5445!
...
### Plot the first LS line: y=payroll, x=avgwin!!!
  par(mgp=c(1.8,.5,0), mar=c(3,3,2,1)) 
  plot(datapay$payroll, datapay$avgwin, 
       pch  = 16, 
       xlab = "Payroll", 
...
# qqline(z)   #shift-command-c  to add or to suppress the pound sign
...
# Make a small movie to see the variability of sample from sample
...
### generate y only each time to see the variability of ls estimates.  
  
  for (i in 1:20) {
  y <- 1 + 2*x + rnorm(100, 0, 2) # generate response y's 
  
### The true line is y=1+2*x, i.e., beta0=1, beta1=2, sigma=2
  ...
  ### The theory says that b_1 follows normal with mean = beta_1=2 and sd= sqrt(var(b_1))
  
  ### var(b_1)= sigma^2 /((n-1)* (sd(x))^2)
  sigma <- 2
  n <- length(y)
