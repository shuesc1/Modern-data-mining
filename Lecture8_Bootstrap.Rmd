...
+ In real life we only have one data sample. We have nice statistics that can be used to estimate some unknown parameters. But we may not have _standard errors_ handy for the estimates. 

+ __Solution__: use bootstrap sample to replicate the statistic. 
+ __The key__: bootstrap sample resembles the sample cdf. 

####  Weibull distribution: life time to an event: 

_Ex_: how long a product survives; how long a patient survives...

__1) Get familiar with Weibull variables__

__Weibull densities are defined as:__
...
+ __Two parameters__:  shape=k and scale=lambda
+ Shape parameter controls the _degree of skewness_. 

 __Take a look at Weibull shape=.5 and lambda=1__
...
__Four Weibull distribution when shape parameters go from .5 to 2__
...
__Let's study X follows Weibull(shape=1.5 and scale=1)__
...
__Can you tell the difference between the two histograms??? Nope__

__Implication__:  + We can get another sample from the population for free!
                  + We can then produce any statistics we want for free.
  
__3) We now going to estimate theta=E(X^3) (the true third moment is 2) using the sample we have. This relates to Kurtosis. __
...
#### Step 2: Generate many bootstrap sample and obtain replicates of theta.hat. 

Here we produce 100 Bootstrap estimates.
...
__What have you observed?__
    
##### FACTS: Bootstrap estimates theta.hat.b follows a Normal dis'n (Coincident? No.) with center to be the original estimate theta.hat=mean(X^3)
...
+ In this case the Bootstrap Percentil ci will work better since the sample median is not exactly unbiased.
...
__When it doesn't work__:
1. If the pivotal estimate is not a good estimate of the unknown parameters  
1. The lasso estimates are biased so Bootstrap CI's wont' work.
          
## Part II: Bootstrap in action          

+ When the linear model shows strong heteroscedasticity, all the inference made based on the model assumptions may not be trust worthy. 
+ We may try to fix it by transformations on y etc. 
+ But we could also do Bootstrap!!!!         
...
__2) Bootstrap CI's for coefficients__
...
__Step VI: Construct bootstrap CI__
...
__Take 100 bootstrap samples and produces b-confidence intervals for each coef's__
